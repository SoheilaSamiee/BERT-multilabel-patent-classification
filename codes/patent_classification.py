...
# --------------------------------------------------
# Pre-trained model path setup and loading
# --------------------------------------------------
# Can get updated to transformer version for built-in 
# download/caching method
...
BERT_PRETRAINED_PATH = Path('../trained_models/bert/uncased_L-12_H-768_A-12/')


# BERT_FINETUNED_WEIGHTS = Path('../trained_model/toxic_comments')

PYTORCH_PRETRAINED_BERT_CACHE = Path('../trained_models/bert/fine_tuned_models')
...
args = {
    ...
    "data_dir": DATA_PATH,
    "task_name": "patent_multilabel",
    "no_cuda": False,
    "bert_model": BERT_PRETRAINED_PATH,
    "output_dir": OUTPUT_PATH,
    "max_seq_length": 128,#32,#64,#128 #512
    "do_train": True,
    "do_eval": True,
    "do_lower_case": True,
    "train_batch_size": 32,
...
class BertForMultiLabelSequenceClassification(BertPreTrainedModel):
    ...
    Inputs:
        ...
        `labels`: labels for the classification output: torch.LongTensor of shape [batch_size, num_labels]
            with binary indices reflecting the activation of each label.
            ------------------------------------------------------------
    Outputs:
        if `labels` is not `None`:
            Outputs are a set of binary CrossEntropy with logits classification loss of the output with the labels.
            (BCEWithLogitsLoss)
        if `labels` is `None`:
...
#         data_df['comment_text'] = data_df['comment_text'].apply(cleanHtml)
    ...
    # Data structure: df should be converted to a list with these rows: [0]: guide (index) - [1]: text - [2:]:label
    # products_list = df.values.tolist() - should be reordered to index, text, label (should be a list of num_classes size -starting from order 2)
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        labels = ''.join(example.labels).split(",")
        indices = [label_map[i] for i in labels]         
        labels_ids = np.zeros(len(label_map))
        labels_ids[indices]=1
...
processors = {
    "patent_multilabel": MultiLabelTextProcessor
}
...
#               'loss': tr_loss/nb_tr_steps,
    ...
    #predicted_values.to_csv(DATA_PATH/'predicted_labels.csv', index=None)
